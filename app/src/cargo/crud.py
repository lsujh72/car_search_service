...
class CRUDCargo(CRUDBase[Cargo, CargoCreate, CargoUpdate]):
    def create(self, db_session: Session, obj_in: CreateSchemaType) -> ModelType:
        ...
        location_id = get_location_id(db_session, obj_in_data.pop('location_pick_up_zip'))
        obj_in_data.update({'location_pick_up_id': location_id})

        location_id = get_location_id(db_session, obj_in_data.pop('location_delivery_zip'))
    ...
    def list(self, db_session: Session, weight: int, miles_to_cargo: int) -> List[CargoList]:
        params = {}
        sql_query = SQL_QUERY
        if weight:
        ...
        for obj in objs:
            ...
            number_nearby_cars = number_of_nearby_cars(db_session, location_pick_up, miles_to_cargo)
            results.append(CargoList(description=obj.description,
                                     weight=obj.weight,
    ...
    def get(self, db_session: Session, id: Any) -> Optional[ModelType]:
        ...
        cars = cars_distance(db_session, location_pick_up,)
        result = CargoGet(cars=cars,
