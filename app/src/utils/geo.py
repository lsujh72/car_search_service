...
def location_cars(db_session: Session) -> list:
    ...
    return [((car.location_current.lat, car.location_current.lng), car.unique_number) for car in cars]
...
def number_of_nearby_cars(db_session: Session, location_pick_up: tuple, miles_to_cargo: int = 450) -> int:
...
def cars_distance(db_session: Session, location_pick_up: tuple) -> list:
    ...
    for location_car in locations:
        ...
        cars.append({'unique_number': unique_number,
                    'distance': _distance})
